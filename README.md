...
카카오톡은 기본 텍스트 템플릿 제한 때문에 200자 요약 1개만 보내고, 전문은 `reports/` 폴더에 HTML로 저장합니다. 버튼은 기본 텍스트 템플릿의 `button_title`로 표시합니다. 전문 페이지로 연결하려면 공개 URL을 `.env`에 설정하세요.

```text
REPORT_BASE_URL=https://your-id.github.io/your-repo
```

카카오 Developers의 Product Link에도 같은 웹 도메인을 등록해야 버튼 링크가 정상 동작합니다. 등록되지 않은 도메인이면 카카오톡 환경에 따라 버튼이 보이지 않을 수 있습니다. 특정 URL 하나로 고정하고 싶으면 `KAKAO_DETAIL_URL`을 사용할 수 있습니다.

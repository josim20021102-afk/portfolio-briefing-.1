   ...
   이유: [기술적 분석 요약] + [뉴스 모멘텀 요약] (핵심만 한 줄로 작성)"""
    ...
    try:
        response = requests.post(
            ...
            json={"contents": [{"role": "user", "parts": [{"text": prompt}]}], "generationConfig": {"maxOutputTokens": 1500, "temperature": 0.3}},
            timeout=40,
        )
...
def send_kakao_message(text: str, detail_url: str | None = None) -> None:
...
def main() -> None:
    ...
    # 카카오톡 전송 여부와 상관없이 무조건 HTML 리포트 생성
    report_path, detail_url = save_detail_report(briefing)
    print(f"상세 리포트 저장 완료: {report_path}")

    # 이후 카카오톡 전송 조건 검사
    if os.getenv("KAKAO_SEND", "true").lower() in {"1", "true", "yes", "y"}:
        send_kakao_message(briefing, detail_url)

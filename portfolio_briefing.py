...
def build_briefing() -> str:
    ...
    # 기술적 지표 + 뉴스 연동 AI 추천
    recommendations = generate_recommendations_with_news(portfolio_signals, market_top5, as_of)

    return "\n\n".join([
        f"[포트폴리오 데일리 브리핑] {as_of.isoformat()} KST",
        "1) 보유 종목 최근 종가\n" + "\n".join(format_price(p) for p in prices),
        "2) 매크로 지표\n" + format_macro(macro),
...
def build_kakao_summary(briefing: str) -> str:
    ...
    for line in lines:
        if line.startswith("- ") and "기준일" in line and ":" in line:
            name_val = line[2:].split(":", 1)
            compact_prices.append(f"{short_names.get(name_val[0].split('(')[0].strip(), name_val[0].split('(')[0].strip())} {name_val[1].split('/')[0].strip()}")

    target_stock = "🔥 주목 종목: 현재 관망 추천"
    for line in lines:
        if line.startswith("🎯 [") and "종가" in line:
            target_stock = "🔥 주목 종목: " + line.split("]")[0].replace("🎯 [", "").strip()
            break

    return "\n".join([title, "💰 " + ", ".join(compact_prices), target_stock, "📊 상세 지표 및 뉴스 분석은 아래 클릭!"])[:200]
...
def save_detail_report(briefing: str) -> tuple[str, str | None]:
    ...
    with open(path, "w", encoding="utf-8") as handle:
        handle.write(f"<!doctype html>\n<html lang=\"ko\">\n<head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><title>포트폴리오 데일리 브리핑</title><style>body{{font-family:system-ui,-apple-system,sans-serif;line-height:1.55;margin:24px;max-width:860px}}h1{{font-size:22px}}main{{white-space:normal}}</style></head>\n<body><h1>포트폴리오 데일리 브리핑</h1><main>{body}</main></body></html>\n")
    explicit_url = os.getenv("KAKAO_DETAIL_URL", "").strip()
    if explicit_url: return path, explicit_url
    base_url = os.getenv("REPORT_BASE_URL", "").strip().rstrip("/")
    return path, f"{base_url}/{filename}" if base_url else None

def refresh_kakao_access_token() -> str:
    rest_key, refresh_token = os.getenv("KAKAO_REST_API_KEY", "").strip(), os.getenv("KAKAO_REFRESH_TOKEN", "").strip()
    if not rest_key or not refresh_token: raise RuntimeError("API 키 누락")
    payload = {"grant_type": "refresh_token", "client_id": rest_key, "refresh_token": refresh_token}
    if secret := os.getenv("KAKAO_CLIENT_SECRET", "").strip(): payload["client_secret"] = secret
    response = requests.post("https://kauth.kakao.com/oauth/token", data=payload, timeout=20)
    if not response.ok: raise RuntimeError("토큰 갱신 실패")
...
def send_kakao_message(text: str) -> None:
    ...
    link_url = detail_url or os.getenv("KAKAO_FALLBACK_URL", "https://finance.yahoo.com").strip()
    response = requests.post("https://kapi.kakao.com/v2/api/talk/memo/default/send", headers={"Authorization": f"Bearer {access_token}"}, data={"template_object": json.dumps({"object_type": "text", "text": build_kakao_summary(text), "link": {"web_url": link_url, "mobile_web_url": link_url}, "button_title": "자세히 보기"}, ensure_ascii=False)}, timeout=20)
    response.raise_for_status()
...
def main() -> None:
    ...
    print(briefing)
    if os.getenv("KAKAO_SEND", "true").lower() in {"1", "true", "yes", "y"}: send_kakao_message(briefing)

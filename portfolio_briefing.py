...
def fetch_macro_data() -> dict:
    """환율(USD/KRW), 미국 10년물 금리(^TNX), 한국 코스피(^KS11) 지수를 수집합니다."""
    macro: dict = {}
    symbols = {
        "USD/KRW 환율": "KRW=X",
        "미국 10년물 금리(%)": "^TNX",
...
def analyze_risk_with_gemini(
    ...
    """Gemini AI로 포트폴리오 리스크 종합 분석 코멘트를 생성합니다."""
    ...
    price_text = "\n".join(
        f"- {p.name}({p.ticker}): {p.close:,.2f} {p.currency} ({p.date.isoformat()})"
        for p in prices
    )
...
def fetch_market_top5() -> list[TechnicalSignal]:
    """RSI 기반으로 상승 후보 / 하락 주의 종목 Top5 스크리닝."""
    signals = []
    ...
    bullish = sorted(
        [s for s in signals if s.rsi is not None and s.rsi <= 40],
        key=lambda s: s.rsi,
    )[:3]
    bearish = sorted(
        [s for s in signals if s.rsi is not None and s.rsi >= 65],
        key=lambda s: -s.rsi,
    )[:2]
...
def analyze_stocks_with_gemini(
    ...
    def sig_text(sigs: list[TechnicalSignal]) -> str:
        ...
        for s in sigs:
            ...
            lines.append(
                f"- {s.name}({s.ticker}): 종가 {s.close:,.2f}, "
                f"RSI {s.rsi}, 5MA {s.ma5}, 20MA {s.ma20}, "
                f"등락 {s.change_pct}%, 거래량비율 {s.volume_ratio} {ma_cross}"
            )
        return "\n".join(lines) or "- 없음"
...
※ 이 분석은 투자 참고용이며 투자 결정의 책임은 본인에게 있습니다."""
...
def format_technical(signals: list[TechnicalSignal]) -> str:
    ...
    for s in signals:
        ...
        lines.append(
            f"- {s.name}({s.ticker}): RSI {s.rsi}{rsi_label} | "
            f"5MA {s.ma5} / 20MA {s.ma20} | 등락 {change_str} | 거래량비율 {s.volume_ratio}"
        )
    return "\n".join(lines)
